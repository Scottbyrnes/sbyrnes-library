...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <!--<p>Debug famousOrwell: {{ famousOrwell }}</p>-->
      <ul>
        <li v-for="work in famousOrwell" :key="work.id">
          {{ work.title }}
        </li>
      </ul>
    ...
    <section class="lab-section">
      ...
      <!-- TODO: CODE TO RENDER LIST OF OPENING HOURS HERE -->
       <ul>
        <li v-for="([day, hours], index) in Object.entries(bookstores.openingHours)" :key="index">
          {{ day }} ({{ hours }})
        </li>
      ...
      <p>We operate in: <ul>
        <li v-for="(country, index) in bookstores.countries" :key="index">
          {{ country }}
        </li>
      </ul></p>
    ...
    <section class="lab-section">
      ...
      <button @click="showMessage = !showMessage">Click me to toggle message</button>
      <p v-if="showMessage" class="message success">
        ✨ You're a Vue superstar! ✨</p>
      <p v-else class="message">
        Click the button to see a message.</p>
    </section>

    <section class="lab-section">
      <h2>Attribute, Class and Style Binding with <code>v-bind</code></h2>
      <p>Highlighting Specific Authors:
        <ul>
...
const famousOrwell = computed(() =>
      authors.find((author) => author.name == "George Orwell")?.famousWorks
      );
</script>
